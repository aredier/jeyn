...
class NewInputDataCatalog(jeyn.catalogs.DataCatalog):
    mean_texture = jeyn.catalogs.Feature(name="mean texture", dtype=jeyn.catalogs.Dtypes.FLOAT32, shape=(-1, 1))
    mean_perimeter = jeyn.catalogs.Feature(name="mean perimeter", dtype=jeyn.catalogs.Dtypes.FLOAT32, shape=(-1, 1))
...
def init_dataset(store: jeyn.datasets.DatasetStore):
    ...
    new_dataset_formula = store.get_fromula(
        formula_cls=jeyn.datasets.formulas.StreamingFormula,
        name="test_formula",
        version="0.2.0"
    )
    if new_dataset_formula is None:
        new_dataset_formula = jeyn.datasets.formulas.StreamingFormula(
            "test_formula", version="0.2.0", output_catalog=NewInputDataCatalog(), data_dir=dataset_path
        )
        new_dataset_formula.save()
...
if __name__ == '__main__':
    ...
    model.fit(dataset.loc[:, ["mean texture", "mean perimeter"]], dataset["targets"])
    ...
    checkpoint = jeyn.models.ModelCheckpoint(
        ...
        input_catalog=NewInputDataCatalog(),
    ...
    dataset = pd.read_csv(training_data.files[-1])
    print(new_model.predict(dataset.loc[:, ["mean texture", "mean perimeter"]]))
